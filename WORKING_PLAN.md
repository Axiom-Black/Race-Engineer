...
| S5 | Core loop: upload → client-side parse → summaries + downsampled traces persist → SessionReport/TrackMap dashboard renders | 🟨 In progress | The real COTA triple (local only) uploads and renders a complete session view in < 10 s; unreliable/empty channels display SIM/EMPTY flags | **Front half done 11 Aug** — onboarding spine shipped: Supabase Auth email+password sign-up/sign-in/sign-out (`src/lib/auth.jsx`), auth-gated app shell + empty-state dashboard replacing the S4 smoke screen. **Back-half plan** committed (`docs/S5_IMPLEMENTATION_PLAN.md`, 6 steps). **Step 1 done 12 Aug** — pilot schema committed as `supabase/migrations/0001_pilot_schema.sql` (+ `0002_storage.sql`); Ring 3 acceptance wired into CI and verified locally on Postgres 16 (isolation 0-rows, spoofed-insert rejected, incomplete `complete` rejected, dedup rejected). Remaining: Step 2 ingest module → Step 3 upload flow → Step 4 SessionReport (4 tabs) → Step 5 demo seed. |
