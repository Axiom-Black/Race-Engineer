...
| 12 Aug 2026 | Note | **Test-account friction on the shared Supabase project.** Free-tier SMTP has a low send-rate-limit; a few real sign-up attempts through the UI exhausted it and blocked further real signups for a period. Worked around for automated verification by creating pre-confirmed test users directly via SQL (`pgcrypto`'s `crypt()`/`gen_salt('bf')`, matching Supabase Auth's bcrypt scheme) — legitimate for scripted testing, not a habit to build. Recommended for anyone (teammates included) needing test accounts going forward: turn off "Confirm email" in the Supabase dashboard's Auth settings for this pre-launch pilot (self-serve signup, sidesteps the SMTP limit entirely; flip back on before real launch since RLS is the actual security boundary, not email confirmation) — or use the dashboard's own "Add user" button, which supports creating a pre-confirmed user without SQL. Not yet actioned; needs a decision from Tosin. |
