...
| --- | --- | --- |
| 15 Sep 2026 | PR #63 merged; this session closed on its own row ($121.82, 18.2x) — and the measuring apparatus found to have two defects | **A measurement taken twice on the same subject is a row that can be double-booked, and the ledger's own format invited it.** This session was measured mid-flight at $116.40/370 messages and again at close at $121.82/396. Both are correct readings of one session; appending both books it as two and overstates the total by 96%. The table keys on session id and nothing in it said so, because **the first row of a ledger is always written when there is only one, which is exactly when the uniqueness rule looks unnecessary.** The habit: when you add the first row to a record, write down what makes a row unique — the duplicate is invisible once it is in the table and reads as evidence. **Second, and more interesting: the instrument is inside the system it measures.** Two runs four minutes apart returned $121.76 and $121.82 — *the second run billed for reading the first*. The magnitude is negligible and the property is not: a session's cost is **as at the moment it was taken**, never a closed total, and re-running always nudges it up. **Any measurement whose act of measuring is itself a cost has no final value, only readings** — so take one, at a defined moment, and label the moment rather than pretending to a total. **Third, the session-level number hid the finding.** One figure ($0.31/message) averaged a thread that cost **$0.21/message on day two and $0.46/message on day six** for comparable work. The aggregate is true and says nothing; the gradient is the whole argument for splitting sessions. **When a metric exists to drive a behaviour, disaggregate it along the axis the behaviour changes** — here time-within-session — or it will report an average that no decision can act on. |
| 15 Sep 2026 | Build cost measured for the first time — `scripts/token_report.py` + a committed ledger | **Measure the thing you are spending before you optimise it, and check whether the data is already lying around.** This build ran a month with no cost measurement while every session was writing a JSONL transcript carrying a per-message `usage` block. The estimate it replaced spanned 4.5x ($340–$1,520); the measurement took twenty minutes. **Before commissioning telemetry, grep the tool you are already using for the number.** **The finding inverted where I expected the cost to be: output was 5% of spend and cache reads were 67%.** At 97.9% of input and an 18.4x multiplier over output, the bill is not what the model produces, it is what the conversation re-reads. **The generalisation: in a long-running agent session, cost scales with CONTEXT × TURNS, not with work done** — so the first lever is session length, and model choice is nearly the last (a free model would cut ~5%). **Watch for the metric that inverts your intuition; it is the one that changes behaviour.** **Second, an architectural point about ephemeral environments.** A cloud session gets a fresh container, so a measurement not committed before the session ends is not delayed — it is destroyed. The script is worthless without the committed ledger beside it, and the ritual step that appends the row is the actual deliverable. **When the measuring instrument lives somewhere more ephemeral than the thing it measures, the durable record IS the feature.** **Third, on labelling:** the figures are API-list-*equivalent*, not money spent, because a subscription bills against plan limits rather than per token. Same class of error as the `$0/month` hosting premise — a notional cost and an invoice are different claims, and the moment one is quoted as the other somebody reasons about unit economics from a number that was never charged. |
| 14 Sep 2026 | PR #50 reviewed against primary sources, merged with `main`, its single-rate cache-write bug fixed — and a merge parentage destroyed and repaired | **`git reset --soft HEAD~1` past a merge commit keeps the CONTENT and silently destroys the HISTORY.** Splitting one file out of a merge commit that way drops `MERGE_HEAD`, so the recommit has one parent instead of two. Everything looks right locally — the working tree is correct and `git diff main..head` is clean — while the merge-base stays pinned weeks behind, and the forge three-way merges against a base the branch already absorbed. **The symptom is the tell: GitHub called the PR conflicted and showed 57 files / +5,640 where the real change was 7 files / +239.** When a forge's idea of a diff is wildly larger than your own, suspect the merge-base before the diff. `git merge-base main HEAD` is the one-command check, and `git merge -s ours main` records the parent without touching a tree that is already correct — legitimate *only* after proving the tree already contains the base in full, which the two-dot diff does. **Second, and more portable: verify an outside claim against the primary source, not against its own confidence.** PR #50 read as authoritative and every one of its five claims held — but two were checkable in seconds and one was not checked by its own author (its token 403'd on the Vercel team, so that half was testimony). The cheap re-check turned testimony into evidence and cost nothing. **Third, the defect the review actually found came from reading the reference rather than the diff:** cache writes are 1.25x at the 5-minute TTL and 2x at the 1-hour TTL, and a single hardcoded rate understates the long TTL by 60%. Nothing in the diff looked wrong — the constant was correct for the default. **A plausible constant is the hardest kind of wrong to see, so check constants against the document rather than against the code that uses them**, and prefer splitting a field the way the upstream API reports it (`ephemeral_5m` / `ephemeral_1h`) over collapsing two rates into one number, which makes the cheaper one the silent default. |
...
| 11 Sep 2026 | The Andretti test added to Iteration 5 (D1b) from a hand-annotated circuit map | **The best specification for a feature is an artefact a real practitioner made WITHOUT your product — and it is usually lying around rather than waiting to be commissioned.** A hand-drawn map annotated by a driver with no telemetry says what a track note is for more credibly than any amount of our own reasoning about it, because nothing about it was shaped by what we happen to have built. **The move that makes such an artefact useful is turning it into a probe rather than a mood board:** *reproduce this, from the app alone, and record what you could not express.* That is falsifiable, it runs during the session, and its failures arrive as verbatim sentences instead of as a rating. **Pre-commit the expected outcomes in the same edit**, or the probe gets graded generously after the fact by the person who wrote it. **Second, and more useful than the probe: read the artefact for what it proves you CANNOT do.** Two of the four annotation types on that map — camber and gradient — are not in our channel inventory and one never can be. That is not a backlog item; it is the argument for the feature existing, and an inventory of what your data genuinely cannot say is worth more than another list of what it can. **Third, it re-read one of our own artefacts for us.** The example capture block already contained *"what gear should I actually be in?"* tagged EXTERNAL, and the map showed why that tag survives even though we compute gear at apex: **we record what the driver DID, and the question asks what they SHOULD do.** Those are different products. Whenever a derived value looks like it answers a user's question, check the tense — past-tense measurement and future-tense advice are separated by exactly the work you have not done yet. |
